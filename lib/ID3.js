...
var BitPaddedInt, ByteSpec, EncodedNumericTextSpec, EncodedTextSpec, EncodingSpec, FRAMES, FRAMES_2_2, Frame, ID3, Iconv, MultiSpec, NumericTextFrame, Spec, TextFrame, convertToString, fromLatin1ToString, fromUTF16BEToString, fromUTF16ToString, fromUTF8ToString, fs, latin1ToUTF8, sep, utf16, utf16be, _,
  ...
  ID3.prototype.getFrameReader = function(data, frames) {
    ...
    if (3 <= this.version.majorRev) {
      ...
      return reader = function() {
        var flags, frameSpec, framedata, header, name, offset, size;
        while (true) {
          try {
        ...
        if (!_.isArray(frameSpec)) {
          if (Frame.isValidFrameId(name)) {
            return header + framedata;
          }
        } else {
          return Frame.loadFramedata(_this, frameSpec, flags, framedata);
        }
      };
...
Frame = (function() {
  ...
  function Frame(frameSpecs, data) {
    var odata, spec, value, _i, _len, _ref;
    odata = data;
    for (_i = 0, _len = frameSpecs.length; _i < _len; _i++) {
      spec = frameSpecs[_i];
      if (!(data.length > 0)) {
        throw new Error('ID3JunkFrameError');
      }
      _ref = spec.read(this, data), value = _ref[0], data = _ref[1];
...
Frame.loadFramedata = function(id3, frameSpecs, tflags, data) {
  var frame;
